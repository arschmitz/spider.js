...
var url,
	...
	linkOutputLimit = casper.cli.get( "linkOutputLimit" ) || 10,
	logs = [],
		...
		casper.on( "page.error", function( msg, trace ) {
			...
			var message = {
				...
				functionCall: "function: " +
					( trace[ 0 ] ? trace[ 0 ][ "function " ] : "Not Available" )
			};
...
casper.test.begin( "Go Go Spidey Crawl!", 5, function suite( test ) {
	...
	casper.then( function() {
		...
		if ( check.script ) {
			...
			test.assert( errors.script.length === 0, errors.script.length +
	...
	[ "client", "redirect" ].forEach( function( type ) {
		...
		if ( check[ type ] ) {
			casper.then( function() {
				test.assert( errors[ type ].length === 0,
					errors[ type ].length + " Total " + type + " errors found" );
			} ).then( function() {
				var that = this;

				if ( errors[ type ].length > 0 ) {
					that.echo( errors[ type ].length + " unique " + type +
						" errors found", "ERROR" );
					errors[ type ].forEach( function( error, index ) {
						...
						sourcePages.some( function( page ) {
							currentCount++;
							if ( currentCount > parseInt( linkOutputLimit, 10 ) ) {
								return true;
							}
	...
	casper.then( function() {
		...
		if ( check.resource ) {
			...
			test.assert( errors.resource.length === 0, errors.resource.length +
				" resource errors found" );
		}
	} );
